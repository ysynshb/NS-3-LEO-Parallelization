...
PointToPointChannel::Attach (Ptr<PointToPointNetDevice> device)
{
  ...
  NS_ASSERT (device != 0);
   //std::cout<<"attach======>"<<"     m_nDevices: " <<m_nDevices<<std::endl;
...
{ //std::cout<<"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<std::endl;
  ...
  //std::cout<<m_link[1].m_dst->GetNode ()->GetId ()<<"|"<<m_link[0].m_dst->GetNode ()->GetId ()<<"当前状态"<<m_link[1].m_state<<"|"<<m_link[0].m_state<<std::endl;
  
  if(m_link[1].m_state == STOPPING)
    {   //std::cout<<"发送失败，状态为STOPPING"<<std::endl;
        return false;}
...
void
PointToPointChannel::shuchu(void) 
{  if(m_link[0].m_state==IDLE)
   {NS_LOG_UNCOND("yes");}
   NS_LOG_UNCOND(m_link[0].m_state<<"|"<<m_link[1].m_state);
}
void
PointToPointChannel::setchannelstate(bool channelopen) 
{  if(channelopen==false)
   {
    //std::cout<<m_link[0].m_state<<"|"<<m_link[1].m_state;
    m_link[0].m_state=STOPPING;
    m_link[1].m_state=STOPPING;
    NS_LOG_UNCOND("==>"<<m_link[0].m_state<<"|"<<m_link[1].m_state);
   }else
      {//std::cout<<m_link[0].m_state<<"|"<<m_link[1].m_state;
       m_link[0].m_state=IDLE;
