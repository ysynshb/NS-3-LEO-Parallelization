...
void
PointToPointNetDevice::setdevicestate(bool deviceopen) 
{  if(deviceopen==true )
   {
    //std::cout<<m_link[0].m_state<<"|"<<m_link[1].m_state;
    NS_LOG_UNCOND("m_state: "<<m_state);
     if(m_state == OPEN) {m_state = PRECLOSE; NS_LOG_UNCOND("m_state: "<<"OPEN--->"<<m_state);}
     else if(m_state == PRECLOSE) {m_state = CLOSE;NS_LOG_UNCOND("m_state: "<<"PRECLOSE--->"<<m_state);}
     else if(m_state == CLOSE) {m_state = PREOPEN;NS_LOG_UNCOND("m_state: "<<"CLOSE--->"<<m_state);}
     else if(m_state == PREOPEN) {m_state = OPEN;NS_LOG_UNCOND("m_state: "<<"PREOPEN--->"<<m_state);}
    //m_state=m_state +1;
...
void
PointToPointNetDevice::inidevicestate(bool inidevice) 
{  if(inidevice==true)
   {
    //std::cout<<m_link[0].m_state<<"|"<<m_link[1].m_state;
    m_state=OPEN;
    
    //std::cout<<"ini==>"<<m_state<<std::endl;
   }else
      {//std::cout<<m_link[0].m_state<<"|"<<m_link[1].m_state;
       m_state=CLOSE;
...
uint16_t
PointToPointNetDevice::getdevicestate(void) 
{  
   if(m_state == OPEN)
     {return 1;}
   else if (m_state == PRECLOSE)
     {return 2;}
   else if (m_state == CLOSE)
     {return 3;}
   else if (m_state == PREOPEN)
     {return 4;}
   else
     {return 0;}
