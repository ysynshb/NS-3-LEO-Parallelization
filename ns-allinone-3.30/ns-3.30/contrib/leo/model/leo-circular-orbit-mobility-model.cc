...
LeoCircularOrbitMobilityModel::GetProgress (Time t) const
{
  ...
  int sign = 1;
  //std::cout<<"倾角："<<m_inclination<<std::endl;
  // ensure correct gradient (not against earth rotation)
  if (m_inclination > M_PI/2)
    {
      sign = -1;//std::cout<<"倾角大于90度啦"<<m_inclination<<std::endl;
    }
...
LeoCircularOrbitMobilityModel::RotatePlane (double a, const Vector3D &x) const
{
  ...
  /*std::cout<<"RotatePlane的结果："<<(Product (DotProduct (n, x), n)
    + Product (cos (a), CrossProduct (CrossProduct (n, x), n))
    + Product (sin (a), CrossProduct (n, x)))<<std::endl;*/
...
{ //std::cout<<"calcPosition:"<<std::endl;
  double lat = CalcLatitude ();
  ...
  Vector3D x = Product (m_orbitHeight*1000, Vector3D (cos (m_inclination) * cos (lat),
  			       ...
  			       sin (m_inclination)));
  //std::cout<<RotatePlane (GetProgress (t), x)<<std::endl;
  //std::cout<<"m_orbitHeight*1000:  "<<m_orbitHeight*1000<<"  |vector3D:    "<<x<<"  |m_inclination"<<m_inclination<<"  |lat"<<lat<<std::endl;
...
{ //std::cout<<"海拔："<<(m_orbitHeight - LEO_EARTH_RAD_KM)<<std::endl;//未调用
  return m_orbitHeight - LEO_EARTH_RAD_KM;
}

void LeoCircularOrbitMobilityModel::SetAltitude (double h)
{ //std::cout<<"海拔‘："<<(m_orbitHeight = LEO_EARTH_RAD_KM + h)<<" | h:"<<h<<std::endl;
...
void LeoCircularOrbitMobilityModel::SetInclination (double incl)
{
  ...
  //std::cout<<"SetInclination中的incl："<<incl<<"m_inclination："<<m_inclination<<std::endl;
  Update ();
}
